...
import line
import bullet
from bullet import storage_json
...
@app.route("/cloud_rect/",methods=['GET','POST'])
def cloud_rect():
    type_data = request.get_data().decode('utf8')
    i = 2000
    l_data = {}
    fin_data = []
    while i<=2016:
        l_data['year'] = i
        l_data['type'] = type_data
        l_data['like'] = 0
        i+=1
        fin_data.append(l_data)
        l_data = {}
    # print(fin_data)
    type_db = xunlian.query.all()
    for data in type_db:
        data_types = data.type.split("|")
        data_year = data.years
        data_like = data.like_all
        if data_year =='':
            data_year = 0
        if data_like =='':
            data_like = 0
        data_like = int(data_like)
